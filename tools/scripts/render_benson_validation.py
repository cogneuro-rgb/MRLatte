...
def render_volume_axial(path, title, cmap, vmin, vmax, threshold=None):
    ...
    if threshold is None:
        threshold = 1e-6
    plot_stat_map(
        img, bg_img=os.path.join(ATLAS_DIR, "mni152", "mni152.nii.gz"),
...
def main():
    ...
    lh = np.asarray(nib.load(f"{ATLAS_DIR}/benson14/lh.benson14_angle.v4_0.mgz").dataobj).squeeze()
    rh = -1 * np.asarray(nib.load(f"{ATLAS_DIR}/benson14/rh.benson14_angle.v4_0.mgz").dataobj).squeeze()
    ...
    plt.close(f)
    f = render_volume_axial(f"{ATLAS_DIR}/benson14/benson14_polar_angle.nii.gz",
    ...
    # 2) Eccentricity
    lh = np.asarray(nib.load(f"{ATLAS_DIR}/benson14/lh.benson14_eccen.v4_0.mgz").dataobj).squeeze()
    rh = np.asarray(nib.load(f"{ATLAS_DIR}/benson14/rh.benson14_eccen.v4_0.mgz").dataobj).squeeze()
    ...
    plt.close(f)
    f = render_volume_axial(f"{ATLAS_DIR}/benson14/benson14_eccentricity.nii.gz",
    ...
    lh = np.asarray(nib.load(f"{ATLAS_DIR}/benson14/lh.benson14_varea.v4_0.mgz").dataobj).squeeze().astype(int)
    rh = np.asarray(nib.load(f"{ATLAS_DIR}/benson14/rh.benson14_varea.v4_0.mgz").dataobj).squeeze().astype(int)
    ...
    plt.close(f)
    img = nib.load(f"{ATLAS_DIR}/benson14/benson14_visual_areas.nii.gz")
    fig = plt.figure(figsize=(10, 4), facecolor="#050505")
    plot_roi(img, bg_img=os.path.join(ATLAS_DIR, "mni152", "mni152.nii.gz"),
    ...
    # 4) Wang 2015
    lh = np.asarray(nib.load(f"{ATLAS_DIR}/wang2015/lh.wang15_mplbl.v1_0.mgz").dataobj).squeeze().astype(int)
    rh = np.asarray(nib.load(f"{ATLAS_DIR}/wang2015/rh.wang15_mplbl.v1_0.mgz").dataobj).squeeze().astype(int)
    ...
    plt.close(f)
    img = nib.load(f"{ATLAS_DIR}/wang2015/wang2015_maxprob.nii.gz")
    fig = plt.figure(figsize=(10, 4), facecolor="#050505")
    plot_roi(img, bg_img=os.path.join(ATLAS_DIR, "mni152", "mni152.nii.gz"),
             display_mode="z", cut_coords=6, cmap="tab20",
             figure=fig, black_bg=True,
