...
mni = nib.load(f"{ATLAS_DIR}/mni152/mni152.nii.gz")
pa = nib.load(f"{ATLAS_DIR}/benson14/benson14_polar_angle.nii.gz")
...
nib.save(nib.Nifti1Image(merged, aff), f"{ATLAS_DIR}/benson14/benson14_polar_angle.nii.gz")
...
visf_in = nib.load(f"{ATLAS_DIR}/visfatlas/visfAtlas_maxprob.nii.gz")
...
if visf_in.shape == mni.shape and np.allclose(visf_in.affine, mni.affine):
    ...
    nib.save(nib.Nifti1Image(arr, mni.affine), f"{ATLAS_DIR}/visfatlas/visfAtlas_maxprob.nii.gz")
