...
# --- EDIT THIS: point to your lesion; all outputs are auto-named from it ----- #
DEFAULT_LESION = r"C:\Users\manas\Downloads\lesion_2026-06-12T06-20-27.nii.gz"
DEFAULT_BUNDLE = r"C:\Users\manas\Desktop\Projects\Cophe-NeuroVue-1-bug-small-fixes\Cophe-NeuroVue-1-bug-small-fixes\DaLnm\lnm_bundle_d100.npz"
DEFAULT_THRESHOLD = 7.0
SAVE_RAW = True          # also write the uncorrected t-map for comparison
# ---------------------------------------------------------------------------- #


def _derive_outputs(lesion_path):
    """Auto-name the t-map / raw / mask files next to the lesion, from its stem.
    Handles .nii and .nii.gz.
    """
    folder = os.path.dirname(lesion_path)
    base = os.path.basename(lesion_path)
    ...
    j = lambda suffix: os.path.join(folder, stem + suffix)
    return (j("_lnm_tmap.nii.gz"), j("_lnm_tmap_RAW.nii.gz"),
            j("_network_mask.nii.gz"))
...
class Connectome:
    ...
    # -- main entry -------------------------------------------------------------
    def compute(self, lesion_path, out_path=None, tmap=True, raw_out=None):
        ...
        img = self._to_img(stat_corr)
        if out_path:
            nib.save(img, out_path)
        raw_img = self._to_img(stat)
        if raw_out:
            nib.save(raw_img, raw_out)
        return {"img": img, "raw_img": raw_img,
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=None,
                    help="t-map path (default: auto-named from lesion)")
    ap.add_argument("--raw-out", default=None,
                    help='uncorrected t-map path (default: auto; "" to disable)')
    ap.add_argument("--mask-out", default=None,
                    help="signed mask path (default: auto-named from lesion)")
    ...
    d_out, d_raw, d_mask = _derive_outputs(a.lesion)
    out_path = a.out or d_out
    mask_out = a.mask_out or d_mask
    raw_out = a.raw_out if a.raw_out is not None else (d_raw if SAVE_RAW else "")

    res = Connectome(a.bundle).compute(a.lesion, out_path, tmap=not a.mean,
                                       raw_out=(raw_out or None))
