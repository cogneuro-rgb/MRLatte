...
All atlas files are read from --atlases-dir (default: frontend/public/atlases/).
Everything is already present in the repo — no downloads needed.

Usage:
    python scripts/batch_lesion_full_report.py \\
        --lesions-dir Lesion_Resliced_MNI_108_patients \\
        --output-dir  output_full

    # Override defaults:
        [--atlases-dir frontend/public/atlases]
        [--max-ecc 30]
        [--bin-angle 5]
        [--bin-ecc 1]
...
def main():
    ...
    ap.add_argument("--atlases-dir",  default="frontend/public/atlases",
    ...
    pa_path  = atlases_dir / "benson14_polar_angle.nii.gz"
    ecc_path = atlases_dir / "benson14_eccentricity.nii.gz"
    ...
    visf_data, visf_labels = _load_optional_atlas(
        "visfAtlas_maxprob.nii.gz", "visfAtlas_labels.json", "visfAtlas")
    harv_data, harv_labels = _load_optional_atlas(
        "harvard_oxford_cort.nii.gz", "harvard_oxford_cort_labels.json", "Harvard-Oxford")
    wang_data, wang_labels = _load_optional_atlas(
        "wang2015_maxprob.nii.gz", "wang2015_labels.json", "Wang 2015")

    mni_img = None
    mni_path = atlases_dir / "mni152.nii.gz"
