...
def main():
    ...
    report = ["NeuroVue Benson/Wang 2015 atlas regeneration — v2 KD-tree fill",
              "================================================================",
    ...
    def do_one(label_kind, name_in, fname_out, categorical, expected_desc, expected_check=None):
        print(f"\n== {fname_out} ==")
        lh = load_mgz_scalars(f"{ATLAS_DIR}/lh.{name_in}.mgz")
        rh = load_mgz_scalars(f"{ATLAS_DIR}/rh.{name_in}.mgz")
        ...
        save_nifti(out, aff, f"{ATLAS_DIR}/{fname_out}",
    ...
    # Continuous maps
    do_one("polar_angle", "benson14_angle.v4_0", "benson14_polar_angle.nii.gz",
           categorical=False, expected_desc="0–180° per hemisphere (Benson 2014)")
    do_one("eccentricity", "benson14_eccen.v4_0", "benson14_eccentricity.nii.gz",
           categorical=False, expected_desc="0–90° visual angle (Benson 2014)")
    # Categorical maps
    do_one("visual_areas", "benson14_varea.v4_0", "benson14_visual_areas.nii.gz",
           categorical=True,
           expected_desc="12 labels: 1..12 = V1/V2/V3/hV4/VO1/VO2/LO1/LO2/TO1/TO2/V3b/V3a (Benson 2014)")
    do_one("wang2015", "wang15_mplbl.v1_0", "wang2015_maxprob.nii.gz",
           categorical=True,
           expected_desc="25 labels (Kastner Lab Wang 2015 max-prob)")

    # Soft prob = 6-mm gaussian on Wang binary mask
    print("\n== wang2015_prob.nii.gz (smoothed binary surrogate) ==")
    from scipy.ndimage import gaussian_filter
    wp = nib.load(f"{ATLAS_DIR}/wang2015_maxprob.nii.gz").get_fdata()
    prob = gaussian_filter((wp != 0).astype(np.float32), sigma=2.0)
    save_nifti(prob, aff, f"{ATLAS_DIR}/wang2015_prob.nii.gz", dtype=np.float32)
    report.append("wang2015_prob.nii.gz: gaussian-smoothed binary mask of Wang max-prob")

    # Labels JSON
    with open(os.path.join(ATLAS_DIR, "wang2015_labels.json"), "w") as f:
