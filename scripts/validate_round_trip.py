...
def _validate_atlas(atlas, atlas_dir, surfaces, inv_affine):
    nii_path = Path(atlas_dir) / atlas["nii"]
    ...
    for hemi in ("lh", "rh"):
        ...
        mgz_path = Path(atlas_dir) / f"{hemi}.{atlas['mgz']}.mgz"
...
def run(atlas_dir):
    ...
    ref_vol_path = atlas_dir / "benson14_polar_angle.nii.gz"
    if not ref_vol_path.exists():
        ref_vol_path = next(atlas_dir.glob("*.nii.gz"), None)
...
if __name__ == "__main__":
    atlas_dir = sys.argv[1] if len(sys.argv) > 1 else "/app/frontend/public/atlases"
