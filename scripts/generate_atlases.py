...
Outputs into /app/frontend/public/atlases/:
  visual_areas_v1v5.nii.gz   int16 labels 1..5 (V1, V2, V3V, V4, V5)
  wang2015_maxprob.nii.gz    int16 labels 1..11 (Wang regions, anchored on V1/V2/V3 subdivision)
  wang2015_prob.nii.gz       float32 0..1 probability map
  benson14_polar_angle.nii.gz   float32 0..360 deg, only within visual areas
  benson14_eccentricity.nii.gz  float32 0..90 deg, only within visual areas
...
JUELICH_PATH = os.path.join(ATLAS_DIR, "juelich_atlas.nii.gz")
...
def main():
    ...
    def save(arr, fname, dtype):
        ...
        nib.save(out, os.path.join(ATLAS_DIR, fname))
        nz = int((arr != 0).sum())
        rng = f"{arr.min():.2f}..{arr.max():.2f}" if arr.dtype.kind == "f" else f"{int(arr.min())}..{int(arr.max())}"
        print(f"  ✓ {fname} | nz={nz:,} | range={rng}")

    print("Saving…")
    save(varea, "visual_areas_v1v5.nii.gz", np.int16)
    save(polar, "benson14_polar_angle.nii.gz", np.float32)
    save(eccen, "benson14_eccentricity.nii.gz", np.float32)
    save(wang, "wang2015_maxprob.nii.gz", np.int16)
    save(prob, "wang2015_prob.nii.gz", np.float32)
    # Also keep old name for backward-compat link
    save(varea, "benson14_visual_areas.nii.gz", np.int16)
    print("Done.")
