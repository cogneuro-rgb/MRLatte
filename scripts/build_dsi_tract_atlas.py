...
atlas + labels JSON that NeuroVue's atlas infrastructure consumes (same shape as
...
-----
    ...
    python scripts/build_dsi_tract_atlas.py --prefix HCP1065 \\
        --src-dir HCP1065/tracts --names-xlsx HCP1065/abbreviation.xlsx \\
        --resample-to frontend/public/atlases/mni152.nii.gz

    # HCP842 (4D file), reproduce the existing build:
    python scripts/build_dsi_tract_atlas.py --prefix HCP842 \\
        --src-4d HCP824/HCP842_tractography.nii.gz \\
        --src-names HCP824/HCP842_tractography.txt \\
        --resample-to frontend/public/atlases/mni152.nii.gz

Output (into --out-dir, which is ATLAS_DIR shared by frontend + backend)
------
...
def main():
    ...
    ap.add_argument("--resample-to", default=None,
                    help="Reference NIfTI (e.g. the app's mni152.nii.gz) to resample "
                         "every tract mask onto, nearest-neighbour.")
    args = ap.parse_args()
    build(args.prefix, args.src_4d, args.src_dir, args.src_names, args.names_xlsx,
          args.out_dir, args.resample_to, args.threshold)
